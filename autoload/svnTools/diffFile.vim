" Script Name: svnTools/diffFile.vim
 "Description: 
"
...
function! svnTools#diffFile#GetModifiedFilesList()
...
" Cmd: SvnDiffVdr
function! svnTools#diffFile#OpenVimDiffGetFilesAndRevisionsEveryIndexLine(...)
    let l:res = svnTools#tools#isSvnAvailable()
    if l:res != 1
        call svnTools#tools#Error("ERROR: ".l:res)
        return
    ...
    if &filetype !=# 'diff'
      call svnTools#tools#Warn("Attention, file type is not diff!")
      call input("")
    endif
...
" Cmd: Svndvdr
function! svnTools#diffFile#OpenVimDiffOnEachFileAndRevision()
    let l:res = svnTools#tools#isSvnAvailable()
    if l:res != 1
        call svnTools#tools#Error("ERROR: ".l:res)
        return
    ...
    if expand("%") !~ "_r[1-9].*\.diff"
        echo "First launch commands: Svnr, to get the revision diff."
        call svnTools#tools#Warn("Current file is not an svn diff file!")
    ...
    if len(l:list) != 1
        call svnTools#tools#Error("Could't find the revision number!")
        return
    endif
...
" Cmd: SvnDiffVdr
function! svnTools#diffFile#OpenVimDiffOnAllFiles(...)
    let l:res = svnTools#tools#isSvnAvailable()
    if l:res != 1
        call svnTools#tools#Error("ERROR: ".l:res)
        return
    ...
    for l:line in l:linesList
        if l:line[0:3] == "+++ "
            if l:line =~ "(working copy)"
                let l:list = split(l:line)
                let l:file = l:list[1]
                call svnTools#vimdiff#File(l:file)
            else
                call svnTools#diffFile#OpenVimDiffGetFileAndRevisionFromCurrentLine()
            endif
        endif
    endfor
endfunction
...
" Arg2: [optional] revision2
" Cmd: SvnDiffVdrf
function! svnTools#diffFile#OpenVimDiffGetFileAndRevisionFromCurrentLine(...)
    let l:res = svnTools#tools#isSvnAvailable()
    if l:res != 1
        call svnTools#tools#Error("ERROR: ".l:res)
        return
    ...
    if &filetype !=# 'diff'
      call svnTools#tools#Warn("Attention, file type is not diff! (Press key to continue)")
    endif
    ...
    elseif a:0 == 1
        ...
        let l:check = @z
        if l:check != "(revision"
            call svnTools#tools#Error("Verify cursor line position. Place on line ending with: '(revision xxxx)'.")
            return
        endif
    ...
    if file == ""
        ...
        elseif l:tag1 == "Index:" || l:tag1 == "---" || l:tag1 == "+++"
            ...
            call svnTools#tools#Error("Line format error. Verify cursor line position.")
            return
        endif
    ...
    if l:file == ""
        call svnTools#tools#Error("File path not found on current file. Check cursor line position")
        return
    endif
